...
export class EdgeExchange {
    ...
    constructor(readonly removals: ReadonlySet<Edge>, readonly insertions: ReadonlySet<Edge>) {
        if (removals.size != insertions.size)
            throw new InvalidEdgeExchangeError(`The number of edges to remove should be equal to the number of edges to insert, got ${removals.size} removals and ${insertions.size} insertions.`)
    }
}
